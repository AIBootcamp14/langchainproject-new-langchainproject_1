...
다음 4가지 항목을 각각 1-5점으로 평가하세요:

1. 정확성(Accuracy): 답변이 사실적으로 정확하고 오류가 없는가?
    - 1점: 명백한 오류나 잘못된 정보 포함
    - 2점: 부정확한 정보가 다수 포함
    - 3점: 대체로 정확하나 일부 불확실한 부분 존재
    - 4점: 정확하고 신뢰할 수 있는 정보
    - 5점: 매우 정확하고 검증 가능한 정보

2. 완전성(Completeness): 질문에 충분히 답변했는가?
    - 1점: 질문의 핵심을 전혀 다루지 않음
    - 2점: 질문의 일부만 답변
    - 3점: 주요 내용은 포함하나 세부사항 부족
    - 4점: 질문에 충분히 답변함
    - 5점: 질문을 완벽하게 다루고 추가 정보도 제공
    
3. 관련성(Relevance): 답변이 질문과 직접적으로 관련되어 있는가?
    - 1점: 질문과 무관한 내용
    - 2점: 관련성이 약함
    - 3점: 대체로 관련있으나 불필요한 내용 포함
    - 4점: 질문과 직접 관련된 내용
    - 5점: 질문에 완벽하게 부합하는 내용
    
4. 명확성(Clarity): 답변이 이해하기 쉽고 명확한가?
    - 1점: 이해하기 매우 어려움
    - 2점: 모호하거나 혼란스러운 표현 다수
    - 3점: 이해 가능하나 개선 필요
    - 4점: 명확하고 이해하기 쉬움
    - 5점: 매우 명확하고 체계적으로 작성됨
    
중요: 반드시 아래 형식으로만 답변하세요. 추가 설명이나 코멘트는 작성하지 마세요.
